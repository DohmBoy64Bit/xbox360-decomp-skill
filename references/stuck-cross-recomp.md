...
If progress stalls — same crash twice, no new evidence, unregistered guest VA, GPU draw/present failures, boot not advancing — do **not** retry the same local-only patch. Gather external evidence first, then re-check local game artifacts. The parent skill’s [debug-triage.md](debug-triage.md) maps symptoms to layers; this file is the mandatory research order before the next patch.
...
| Public layout reference | See [project-templates.md](project-templates.md) | Filesystem and XBLA extraction patterns only — verify locally |
...
- Ghidra / GhidraMCP at the guest PC (see [ghidra-evidence.md](ghidra-evidence.md))
